...
class PRMController:
    ...
    def genCoords(self, maxSizeOfMap=100):
        self.coordsList = np.random.randint(maxSizeOfMap, size=(self.numOfCoords, 2))
    ...
    def checkCollision(self, obs, point):
        if hasattr(obs, "vertices"):
            return self.checkCollisionIrregular(obs.vertices, point)
        elif hasattr(obs, "bottomLeft") and hasattr(obs, "bottomRight"):
            return self.checkCollisionRectangular(obs, point)
        else:
            raise ValueError("Unknown obstacle type")

    def checkCollisionIrregular(self, vertices, point):
        if len(vertices) < 3:
            # Not enough vertices to form a valid polygon
            return False

        # Ensure the last vertex is the same as the first to form a closed polygon
        if not np.array_equal(vertices[0], vertices[-1]):
            vertices = np.vstack([vertices, vertices[0]])

        obstacle_shape = shapely.geometry.Polygon(vertices)
        point_location = shapely.geometry.Point(point[0], point[1])

        return obstacle_shape.contains(point_location)

    def checkCollisionRectangular(self, obs, point):
        p_x, p_y = point
        return (
            obs.bottomLeft[0] <= p_x <= obs.bottomRight[0]
            and obs.bottomLeft[1] <= p_y <= obs.topLeft[1]
        )
